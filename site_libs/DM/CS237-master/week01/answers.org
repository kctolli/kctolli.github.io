#+TITLE: Answers to DM1 Week 00 EPPs
#+LANGUAGE: en
...
#+begin_info
  ...
  that the word \lsquo{}strengths\rsquo has a VCR of 1/8. Other answers (less common words)
  with lower (non-zero) VCRs are acceptable.
#+end_info
** DZB
#+begin_info
...
#+begin_info
  ...
  In addition, the differences between the k^{th} and the (k-1)^{th} Product/6 values
  are k^2, the squares.
#+end_info
** CJF
#+begin_info
  ...
  times (treating the first as AM, the second as PM).
